...
class TvPurchasePage extends StatefulWidget {
  const TvPurchasePage({super.key});

  @override
  State<TvPurchasePage> createState() => _TvPurchasePageState();
}

class _TvPurchasePageState extends State<TvPurchasePage> {
  static const _category = 'tv';
  ...
  String get _currentKey =>
      '$_serviceId|${_cardController.text.trim()}|$_variationCode';

  bool get _isVerified =>
      _verifiedName != null && _verifiedKey == _currentKey;

  void _onProviderChanged(String? val) {
    setState(() {
      _serviceId = val;
      _variations = [];
      _variationCode = null;
    ...
    if (val != null) {
      context.read<BillsBloc>().add(LoadVariationsEvent(_category, val));
    }
  }

  void _verify() {
    if (_serviceId == null) {
      _snack('Select a TV provider first');
      return;
    }
    if (_cardController.text.trim().isEmpty) {
      _snack('Enter your smartcard/IUC number');
      return;
    }
    context.read<BillsBloc>().add(VerifyCustomerEvent(
          serviceId: _serviceId!,
          customerId: _cardController.text.trim(),
          variationCode: _variationCode ?? '',
        ));
  }

  Future<void> _pay() async {
    if (!(_formKey.currentState?.validate() ?? false)) return;
    if (_serviceId == null || _variationCode == null) {
      _snack('Select a provider and package');
      return;
    }
    if (!_isVerified) {
      _snack('Verify the smartcard number first');
      return;
    }
    ...
    final confirmed = await ConfirmationDialog.show(
      ...
      message:
          'Pay ${CurrencyFormatter.formatNaira(_amount)} for $_variationName on card ${_cardController.text.trim()} ($_verifiedName)?',
      ...
      body: BlocListener<AuthBloc, AuthState>(
        ...
        child: BlocConsumer<BillsBloc, BillsState>(
          listener: (context, state) {
            ...
            } else if (state is VariationsSuccess) {
              setState(() => _variations = state.variations);
            } else if (state is VariationsFailure) {
          ...
          builder: (context, state) {
            ...
            final variationsLoading = state is VariationsLoading;
            ...
            return Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _label('Package'),
                  const SizedBox(height: 10),
                  _packageDropdown(variationsLoading),
                  const SizedBox(height: 20),
                  _label('Smartcard / IUC Number'),
                  const SizedBox(height: 10),
                  _cardField(verifying),
                  if (_isVerified) ...[
  ...
  Widget _packageDropdown(bool loading) {
    if (loading) {
      return Container(
        height: 56,
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _variations.map((v) {
        ...
        return DropdownMenuItem<String>(
          ...
          child: Text(
            '${v['name']} — ${CurrencyFormatter.formatNaira(amount)}',
            overflow: TextOverflow.ellipsis,
          ),
        );
      }).toList(),
      onChanged: (val) {
        ...
        setState(() {
          _variationCode = val;
          _variationName = v['name']?.toString() ?? '';
          _amount = double.tryParse(v['amount'].toString()) ?? 0;
          _verifiedName = null;
        });
      },
  ...
  Widget _cardField(bool verifying) {
    return TextFormField(
      ...
      validator: (v) =>
          (v == null || v.trim().isEmpty) ? 'Enter smartcard number' : null,
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _summaryRow('Package', _variationName),
          _summaryRow('Amount', CurrencyFormatter.formatNaira(_amount)),
        ],
      ),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(processing ? 'Payment Processing' : 'Subscription Active!',
              ...
              const SizedBox(height: 20),
              _detailRow('Package', _variationName),
              _detailRow('Smartcard', state.customerId),
